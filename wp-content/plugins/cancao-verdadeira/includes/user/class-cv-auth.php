...
class CV_Auth {
    ...
    public static function shortcode_login( $atts ) {
        ...
        <style>
        .cv-auth-box { max-width:420px; margin:0 auto; background:#222; border-radius:12px; padding:36px 32px; box-shadow:0 8px 32px rgba(0,0,0,.4); }
        .cv-auth-header { text-align:center; margin-bottom:28px; }
        .cv-auth-icon { font-size:36px; display:block; margin-bottom:10px; }
        .cv-auth-title { color:#F5F0E0; font-family:'Oswald',sans-serif; font-size:22px; margin:0 0 6px; }
        .cv-auth-subtitle { color:#999; font-size:14px; margin:0; }
        .cv-auth-field { margin-bottom:18px; }
        .cv-auth-field label { display:block; color:#C8B98A; font-size:13px; font-weight:600; margin-bottom:6px; }
        .cv-auth-field input[type="email"],
        .cv-auth-field input[type="text"],
        .cv-auth-field input[type="password"] { width:100%; padding:11px 14px; background:#1a1a1a; border:1px solid #444; border-radius:7px; color:#F5F0E0; font-size:15px; box-sizing:border-box; transition:border-color .2s; }
        .cv-auth-field input:focus { border-color:#D4A017; outline:none; box-shadow:0 0 0 2px rgba(212,160,23,.15); }
        .cv-auth-password-wrap { position:relative; }
        .cv-auth-password-wrap input { padding-right:44px; }
        .cv-auth-eye { position:absolute; right:10px; top:50%; transform:translateY(-50%); background:none; border:none; cursor:pointer; font-size:16px; padding:4px; color:#888; }
        .cv-auth-remember { display:flex; justify-content:space-between; align-items:center; margin-bottom:22px; font-size:13px; color:#C8B98A; }
        .cv-auth-remember label { display:flex; align-items:center; gap:6px; cursor:pointer; }
        .cv-auth-link { color:#D4A017; text-decoration:none; }
        .cv-auth-link:hover { color:#FFD700; text-decoration:underline; }
        ...
        .cv-auth-divider { text-align:center; color:#555; margin:18px 0; font-size:13px; position:relative; }
        .cv-auth-divider::before, .cv-auth-divider::after { content:''; position:absolute; top:50%; width:42%; height:1px; background:#333; }
        .cv-auth-divider::before { left:0; } .cv-auth-divider::after { right:0; }
        .cv-auth-switch { text-align:center; color:#888; font-size:14px; margin:0; }
        .cv-auth-msg { padding:12px 16px; border-radius:7px; font-size:14px; margin-bottom:18px; }
        .cv-auth-msg.success { background:#1a3a1a; color:#5cb85c; border:1px solid #2d6a2d; }
        .cv-auth-msg.error   { background:#3a1a1a; color:#e74c3c; border:1px solid #6a2d2d; }
        .cv-auth-terms { font-size:12px; color:#666; margin-top:14px; text-align:center; }
        .cv-auth-strength { height:4px; border-radius:2px; margin-top:6px; transition:all .3s; background:#333; }
        .cv-auth-strength.weak   { background:#e74c3c; width:33%; }
        .cv-auth-strength.medium { background:#f39c12; width:66%; }
    ...
    public static function shortcode_register( $atts ) {
        ...
        <div class="cv-auth-box" id="cv-register-box">
            ...
            <div class="cv-auth-form">
                ...
                <div class="cv-auth-field">
                    <label for="cv-register-password">Senha <span style="font-weight:400;color:#666">(mín. 8 caracteres)</span></label>
                    <div class="cv-auth-password-wrap">
                ...
                <div class="cv-auth-field">
                    <label for="cv-register-genre">Gênero favorito <span style="font-weight:400;color:#666">(opcional)</span></label>
                    <select id="cv-register-genre" style="width:100%;padding:11px 14px;background:#1a1a1a;border:1px solid #444;border-radius:7px;color:#F5F0E0;font-size:15px;box-sizing:border-box;">
